...
\ifPDFTeX%
  ...
  % Fail early: pgf graphdrawing requires LuaTeX; prevent accidental pdflatex runs
  \PackageError{particle-music}{This document requires LuaLaTeX to draw graphs (pgf graphdrawing).\MessageBreak Please compile with lualatex instead of pdflatex}{Use lualatex}
...
\setlength{\emergencystretch}{4em} % prevent overfull lines (increased from 3em)
\overfullrule=5pt
...
\preto\subsubsection{\phantomsection}
...
\AtBeginDocument{\tikzset{graphs/simple necklace layout/.append style={node distance=0pt,node sep=0pt}}}
...
\hypersetup{
  ...
  hypertexnames=false, % avoid duplicate destination names (page.1) in PDF
  pdfencoding=unicode % use Unicode for bookmarks
}
...
Este documento es un intento de \emph{entender cómo funciona la música} desde un punto de vista explícitamente estructural: qué información aporta un conjunto de notas (acorde/escala) y cómo esa información guía la expectativa del oyente.

La teoría musical tradicional es muy eficaz como \emph{notación y oficio} (permite escribir, interpretar y transmitir música), pero es menos útil cuando buscamos explicaciones: con demasiada frecuencia aparecen respuestas del tipo ``\emph{se hace así}'', ``\emph{queda mejor}'' o ``\emph{tal compositor lo hizo}''.

La hipótesis de partida es simple: muchas sensaciones armónicas pueden describirse como consecuencias de relaciones internas (simetrías, distancias, presencia de tritonos, etc.). En lugar de tomar como axiomático que una nota es ``fundamental'' porque produce descanso, intentaremos describir qué condiciones producen esa sensación.

A lo largo del texto se proponen dos herramientas:
\begin{itemize}
	\tightlist%
	\item una notación gráfica/numérica para ver patrones (simetrías, tritonos, ambigüedades),
	\item un vocabulario mínimo de ``partículas'' para nombrar estructuras sin depender de terminología histórica.
\end{itemize}

El objetivo no es sustituir el lenguaje musical existente, sino aportar un marco que permita:
\begin{itemize}
	\tightlist%
	\item explicar por qué ciertas tríadas sugieren estabilidad o tensión,
	\item comparar escalas por propiedades internas (número y posición de tritonos, ambigüedades, etc.),
	\item describir (y, en lo posible, predecir) efectos típicos de modulación y reorientación.
\end{itemize}

\subsection{Simplificación}

El estudio de la música suele apoyarse en conceptos, palabras, símbolos y notaciones heredados de hace siglos. Con el tiempo, las reglas se han ido superponiendo a medida que los compositores estiraban o rompían las anteriores, y el resultado es un sistema útil para escribir e interpretar, pero a menudo poco claro para explicar.

Aquí intentaremos simplificar: proponer una notación más directa y una teoría más explícita que describa qué información contiene lo que escuchamos, sin recurrir a explicaciones meramente tradicionales.

\subsection{Reglas matemáticas}

En música, muchas regularidades no son arbitrarias: provienen de relaciones físicas y matemáticas entre frecuencias. El objetivo aquí no es dictar qué ``debería'' gustar, sino describir qué estructuras aparecen y qué consecuencias perceptivas suelen tener.

Por eso conviene evitar, en lo posible, etiquetas vagas como ``\emph{suena bien}'' o ``\emph{suena mal}'' cuando estamos analizando. En su lugar, intentaremos explicar qué condiciones producen sensaciones como reposo, tensión, expectativa o ambigüedad.

\subsection{Física y sentido común}

Las reglas físicas del mundo en que vivimos influyen en cómo interpretamos la música. Cuando algo nos suena familiar, intentamos compararlo, etiquetarlo y asimilarlo. Por eso la música no solo describe relaciones internas (intervalos, escalas, acordes), sino que también puede \emph{evocar} escenas y sensaciones del mundo real.

Si los números pudiesen expresar la belleza que esconden, podríamos decir que un número es ``bonito'' por ser primo, por ser la raíz de otro, o por alguna otra propiedad matemática. Pero no parece muy serio llamar ``bonito'' a un número por recordarnos una fecha feliz. Con la música ocurre algo parecido: además de estructura, funciona como lenguaje y dispara asociaciones.

Dicho de otro modo, podemos construir patrones sonoros que se parezcan (estructuralmente) a patrones de ideas o emociones. Igual que con un lápiz podemos trazar algo que un humano reconozca como ``gato'', con música podemos producir configuraciones que sugieran ``alegría'', ``dolor'' o ``vértigo''. A menudo no sabemos explicar por qué lo sentimos, pero el paralelismo estructural está ahí.
...
Las ondas de sonido proceden de elementos físicos que vibran: cuerdas, campanas, tubos, etc. Casi ningún elemento vibra de forma perfectamente senoidal; en su lugar aparece un \emph{timbre}, es decir, una forma de onda resultante de superponer a la vibración principal varios armónicos (vibraciones simultáneas a frecuencias múltiplos de la base). Por eso distinguimos un violín de un saxo.

Cuando dos frecuencias guardan una proporción sencilla (por ejemplo $440$ y $880$ Hz), el oído tiende a integrarlas como un todo estable; cuando la proporción es compleja (por ejemplo $440$ y $457$), percibimos batidos o fricción y el cerebro las separa. En este sentido, las relaciones entre armónicos actúan como el sustrato físico de la armonía.

Si tomamos los armónicos generados a partir de una frecuencia base y los ordenamos, obtenemos un conjunto potencial de alturas. Ese conjunto es el que reutiliza la música: notas relacionadas entre sí por proporciones relativamente simples.

Así pues, una \emph{escala} es un subconjunto de notas seleccionado de ese espacio de relaciones. En la práctica occidental hablamos de 12 notas por octava, y de ahí suelen escogerse subconjuntos (a menudo de 7) que sirven como ``mapa'' para orientar la percepción.
...
Esos acordes están presentes en el 99\% de la música, e incluso, cuando no están presentes, el oyente intenta imaginarlos, como cuando faltan trazos en una imagen y nuestra imaginación completa los que faltan basándose en el sentido común. Cuando escuchamos una tríada mayor o menor admitimos esas ideas por sistema, y, a partir de ellas, intentamos entender qué nos está contando la música, además de orientarnos en el mapa que forman.
...
En la edad media, a ese sonido se le llamaba ``\emph{Diabolus in musicae}'' y parece que representaba algún tipo de desafío para el poder. Solo el nombre ya denota el carácter molesto e incluso peligroso que puede llegar a tener, sobre todo si eres un estado medieval y utilizas la música para mantener adormecida a la masa. Prohibir el tritono es como cuando en el cine el poli malo dice ``Aquí soy yo el que hace las preguntas! ''
...
En realidad, cada nota que se añade tiene una función concreta. Una tríada aporta información precisa sobre el carácter mayor o menor de ese grupo de notas y además una cierta información sobre la posición del mismo en la escala. Esta última no es precisa porque en una escala suele haber varias tríadas mayores y varias menores, así que la posición no está del todo clara. Todas las notas que agreguemos aportarán más información sobre la posición del grado con respecto a la escala y posiblemente alguna idea más como anticipar modulaciones o inducciones al engaño, debilitando ligeramente la sensación inicial.
...
La sensación de ambigüedad entre el carácter mayor/menor de una tríada produce inquietud en el oyente, y supone una especie de adivinanza que requiere una escucha atenta y activa. El mapa no está claro, y el camino es laberíntico, además hay muchas sorpresas esperando en cada cruce. Se necesita una actitud concreta para escuchar música con muchas ambigüedades. El jazz utiliza con profusión la ambigüedad.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{ll>{\raggedright\arraybackslash}Xl}
		...
		Notación      & Nombre Tradicional & Tipo                                        & Notas \\
		\midrule
		\textsf{AR-3} & Eb5                & Mayor con quinta bemol                      & E a b \\
		\textsf{AR-2} & F5+                & Mayor con quinta aumentada                  & F A d \\
		\textsf{AR-1} & adis3b5            & Menor con tercera disminuida y quinta bemol & a b D \\
		\textsf{AR 0} & AM                 & Mayor                                       & A d E \\
		\textsf{AR+1} & bM                 & Mayor                                       & b D F \\
		\textsf{AR+2} & dm                 & Menor                                       & d E a \\
		\textsf{AR+3} & Dm                 & Menor                                       & D F A \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{ll>{\raggedright\arraybackslash}Xl}
		...
		Notación      & Nombre Tradicional & Tipo                                              & Notas   \\
		\midrule
		\textsf{AR-3} & Eb57               & Mayor con quinta bemol y séptima                  & E a b D \\
		\textsf{AR-2} & F5+Maj7            & Mayor con quinta aumentada y séptima mayor        & F A d E \\
		\textsf{AR-1} & adis3b56           & Menor con tercera disminuida quinta bemol y sexta & a b D F \\
		\textsf{AR 0} & Amaj7              & Mayor con séptima mayor                           & A d E a \\
		\textsf{AR+1} & bmaj7              & Mayor con séptima mayor                           & b D F A \\
		\textsf{AR+2} & dm6                & Menor sexta                                       & d E a b \\
		\textsf{AR+3} & DmMaj7             & Menor con séptima mayor                           & D F A d \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{ll>{\raggedright\arraybackslash}Xl}
		...
		Notación      & Nombre Tradicional & Tipo                                                         & Notas     \\
		\midrule
		\textsf{AR-3} & Eb57b9             & Mayor con quinta bemol, séptima y novena bemol               & E a b D F \\
		\textsf{AR-2} & F5+Maj79+          & Mayor con quinta aumentada, séptima mayor y novena aumentada & F A d E a \\
		\textsf{AR-1} & adis3b569          & Menor con tercera disminuida quinta bemol sexta y novena     & a b D F b \\
		\textsf{AR 0} & Amaj7b9            & Mayor con séptima mayor y novena bemol                       & A d E a b \\
		\textsf{AR+1} & bmaj7+9            & Mayor con séptima mayor y novena aumentada                   & b D F A d \\
		\textsf{AR+2} & dm6+9              & Menor sexta con novena aumentada                             & d E a b d \\
		\textsf{AR+3} & DmMaj79            & Menor con séptima mayor y novena                             & D F A d E \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\begin{figure}[H]
	\begin{subfigure}{.29\textwidth}
		...
		\begin{tikzpicture}
			...
			\draw [] (current bounding box.center) circle [radius=28pt];
...
En más resumen todavía: resulta que cambiando \texttt{E} por \texttt{e}, estamos cambiando el carácter de la tríada \texttt{CEG}, de mayor a menor. Estamos cambiando sigilosamente entre alegría y tristeza. Estamos \emph{jugando}.
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.31\textwidth}
		...
		\end{tikzpicture}
		\captionsetup{width=\textwidth}
		\caption{\parbox{0.85\linewidth}{\textsf{RED} reorientada por el oyente para asimilar el nuevo centro tonal en \textsf{E}}}
		\captionsetup{width=\textwidth}
...
Ahora tenemos nombres más eficientes y lógicos para conceptos, como ``Mayor con la quinta aumentada'', ó, uno especialmente chirriante: ``Tríada disminuida''.
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{WHITE} de 1 nota}\label{tab:particles-one-note-white}
\end{table}
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{WHITE} de 2 notas}\label{tab:particles-two-notes-white}
\end{table}
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{WHITE} de 3 notas}\label{tab:particles-three-notes-white}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{WHITE} de 4 notas}\label{tab:particles-four-notes-white}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{WHITE} de 5 notas}\label{tab:particles-five-notes-white}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{WHITE} de 6 notas}\label{tab:particles-six-notes-white}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{WHITE} de 7 notas}\label{tab:particles-seven-notes-white}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|m{1.5cm}|m{9.5cm}|}
		...
		\hline
		                  &                                                                                                                                                                                                                                                 \\
		\iparticle{2,1,1} & Justicia y pregunta. Se emite una sensación de justicia y, simultáneamente se presenta el tritono para plantear un conflicto que se debe resolver                                                                                               \\
		\hline
		                  &                                                                                                                                                                                                                                                 \\
		\iparticle{2,1,2} & Injusticia sobre justicia. La entidad débil es oprimida pero, simultáneamente, es soportada. Doble moral, vicio, sensualidad, indolencia, desgana...
		\\
		\hline
		                  &                                                                                                                                                                                                                                                 \\
		\iparticle{1,2,1} & Justicia sobre injusticia. Ente opresor que actúa simultáneamente como soporte. Modernidad, avance, dinamismo, consumo, adaptación, el poder soporta a sus vástagos débiles apoyándose en otros como ellos, crecimiento, industria, progreso... \\
		\hline
	\end{tabular}
	\caption{Catálogo de partículas provisional}\label{tab:provisional-particle-catalog}
\end{table}

Las dos últimas son especialmente interesantes y ambas están presentes especialmente en la música moderna. La pila \texttt{434} recibe en notación estandar el nombre de \emph{acorde mayor con séptima mayor} y produce una sensación de cierto relax oscuro e indolente, muy apto para baladas románticas, perezosas y sensuales. Si el elemento central de la tríada fuese una persona sería un ser débil que se se apoya sobre uno fuerte pero a su vez siente encima el peso de otro. Un ser débil que siente la contradicción de la \emph{doble moral}, por una parte debería quejarse debido al peso que le oprime, pero por otra disfruta de la solidaridad y el soporte que recibe. Así que permanece en silencio, aceptando su destino y añorando que su situación cambie.

En cuanto a la pila \texttt{343}, en estandar es un acorde \emph{menor con séptima}. En este caso, el elemento central del esquema sería un ser fuerte, que soporta a un ser débil encima, pero que también está apoyado sobre otro debil. Cómo puede sentirse? Si el de la tríada anterior ocultaba su queja por la doble moral, este debe ocultar su alegría, puesto que no está sufriendo, ya que soporta a un ser más débil que él. Puede que se sienta culpable por oprimir a un débil pero diríamos que está en paz consigo mismo, puesto que ya está pagando su deuda. ``Sé que otros sufren por mi culpa, pero ya contribuyo con mis buenas obras, así que olvidemos la injusticia y disfrutemos de la vida''. Esto sugiere civilización, sociedad, avance, optimismo. Un ermitaño que vive en el desierto nunca sentirá esa sensación, su vida diaria no contiene ventajas a costa de que otros sufran, simplemente no hay otros. Si vives en una sociedad moderna, todo funciona gracias a que los servicios que utilizas los pagas con tu trabajo. Otros trabajan para que tú estés cómodo, pero tú también trabajas para que otros lo estén. Así funciona, es la base de la sociedad de consumo. Estos acordes se utilizan profusamente en la música pop, en el jazz, la bossa nova, en el rock, y en general en toda la música moderna. Son poco comunes en música tradicional, incluso en música clásica: parece que suenan demasiado ``modernos''.
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{BLUE} de 1 nota}\label{tab:particles-one-note-blue}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
\begin{table}[H]
	\centering
	...
	\caption{Partículas \sffamily{BLUE} de 2 notas}\label{tab:particles-two-notes-blue}
\end{table}
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{BLUE} de 3 notas}\label{tab:particles-three-notes-blue}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{BLUE} de 4 notas}\label{tab:particles-four-notes-blue}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{BLUE} de 5 notas}\label{tab:particles-five-notes-blue}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{BLUE} de 6 notas}\label{tab:particles-six-notes-blue}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{BLUE} de 7 notas}\label{tab:particles-seven-notes-blue}
\end{table}
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{RED} de 1 nota}\label{tab:particles-one-note-red}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
\begin{table}[H]
	\centering
	...
	\caption{Partículas \sffamily{RED} de 2 notas}\label{tab:particles-two-notes-red}
\end{table}
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{RED} de 3 notas}\label{tab:particles-three-notes-red}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{RED} de 4 notas}\label{tab:particles-four-notes-red}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{RED} de 5 notas}\label{tab:particles-five-notes-red}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{RED} de 6 notas}\label{tab:particles-six-notes-red}
\end{table}
\vspace{-2em} % Ajusta el valor según sea necesario
...
\begin{table}[H]
	...
	\caption{Partículas \sffamily{RED} de 7 notas}\label{tab:particles-seven-notes-red}
\end{table}
...
En resumen: las nuevas tríadas de nuestro vocabulario no tienen un significado específico, simplemente sirven como señales que indican en qué escala estamos (sea cierto o no) con lo que se creará una sensación claridad, oscuridad o desorientación.
...
La sensación de desorientación es definitiva, se produce de manera inequívoca y directa, en cambio, la sensación de oscuridad se produce de manera más gradual: una vez que aparece el primer síntoma, todavía hay alguna probabilidad de que no sea cierta.
...
\begin{table}[H]
	...
	\begin{tabular}{|m{1.5cm}|m{9.5cm}|}
		...
		\hline
		                  &                                     \\
		\iparticle{3,2}   & Tercera disminuida y quinta bemol   \\
		\hline
		                  &                                     \\
		\iparticle{2,3}   & Mayor con quinta bemol              \\
		\hline
		                  &                                     \\
		\iparticle{1,1,2} & Medio disminuido                    \\
		\hline
		                  &                                     \\
		...
		\hline
	\end{tabular}
	\caption{Catálogo de partículas con sus nombres estandar}\label{tab:particle-catalog-estandar}
\end{table}
